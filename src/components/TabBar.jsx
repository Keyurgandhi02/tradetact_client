...
function TabBar({ activeTab, onTabClick }) {
  ...
  return (
    <div className="space-y-5 mt-5 p-4 top-0">
      <div className="border border-black-dark-300 rounded-md bg-black-dark-400">
        <ul className="flex items-center gap-2 text-md font-bold p-2 w-auto">
          {tabs.map((tab) => (
            <li key={tab.path} className="flex-1">
              <NavLink
                to={`${tab.path}`}
                className={`relative flex items-center justify-center gap-2 rounded-md px-3 py-2 text-whiten ${
                  activeTab === tab.path
                    ? "bg-primary"
